# Convocatoria CGR · CPM N° 05-2026-CG

Sitio estático para consultar el **Concurso Público de Méritos N° 05-2026-CG** “Fortalecimiento de los Órganos y Unidades Orgánicas de la Contraloría General de la República” (397 perfiles, 1,049 posiciones).
...
Los filtros y el perfil abierto quedan en la URL, por ejemplo `dashboard.html?perfil=006`.
...
CPM_05_2026_*.md      Texto de las bases y anexos
```

## Regenerar los datos

`js/data.js` se genera a partir del Anexo N° 02 (`.md`) y del Anexo N° 03 (`.md` y `.pdf`). Los PDF no se incluyen en el repositorio: descárgalos del portal de la Contraloría y colócalos en la raíz con sus nombres originales (`CPM_05_2026_Anexo3.pdf`, etc.).

Requiere Node.js y `pdftotext` (incluido en Git para Windows o en Poppler):

```bash
node tools/generar-datos.js
```
